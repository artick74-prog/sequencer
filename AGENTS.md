...
- Swing Grid / master transpose / per-track octave сохраняются в project JSON и local autosave. Piano Roll рисует реально сдвинутые swing-линии, а Snap, Draw, drag, resize и Arrow Left/Right используют те же позиции. **Apply Swing** записывает квантованные swing-позиции непосредственно в note ticks: если в Piano Roll есть выделение, меняются только выделенные ноты, иначе весь проект. Playback и MIDI export больше никогда не добавляют второй swing-слой и используют сохранённые ticks буквально; повторный Apply Swing не накапливает дополнительный swing.
