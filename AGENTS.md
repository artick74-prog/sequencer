...
- На v0.4 Piano Roll имеет `Snap to Grid` с вариантами `1/8`, `1/16`, `1/32`, `1/64` (default `1/16`, сохраняется в localStorage `midi_host_piano_roll_snap_v1`). Этот snap используется для create/move/resize нот и для верхнего Piano Roll ruler. Grid больше не рисует постоянную мелкую сетку: видимая subdivision всегда совпадает с выбранным Snap (`1/8` → сетка восьмыми, `1/16` → шестнадцатыми, `1/32` → тридцать вторыми, `1/64` → шестьдесят четвёртыми), при этом beat/bar остаются визуально сильнее. Верхний ruler можно click/drag'ом двигать по выбранному snap; если transport играл, он временно останавливается и продолжает с новой позиции после отпускания. Новые ноты создаются длиной текущего snap и velocity 100. Move/resize остаются внутри границ Project Clip; правка помечает clip как `edited`/`editedAt`, сохраняя source/provenance. Следующая итерация — multi-select, copy/paste, transpose, undo/redo и дальнейшие editor controls.
