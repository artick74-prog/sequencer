...
- В верхнем transport MIDI Host есть **Swing 0–50%** и **Master Transpose ±36 semitones**. Swing повторяет логику Chiptune Sequencer: задерживает нечётные позиции сетки 1/16; исходные MIDI ticks не переписываются. Master Transpose применяется ко всем pitched tracks, но не к MIDI channel 10 / drum-percussion roles.
...
- Swing / master transpose / per-track octave сохраняются в project JSON и local autosave. Playback и MIDI export используют эти transforms неразрушающе: исходные note numbers/ticks в project JSON не меняются, а экспортированный MIDI соответствует слышимому swing/transpose.
...
- Track header имеет три стабильных по высоте ряда: **name + MIDI channel**, затем отдельный **GM Program slot**, затем **S/M + octave + Edit/Preview/MIDI**. У GM-дорожек во втором ряду показывается полный список GM 0–127; у аппаратных дорожек этот ряд остаётся пустым, чтобы все track lanes имели одинаковую высоту.
...
- Нижний Piano Roll можно закрыть и менять по высоте drag'ом верхнего разделителя; высота сохраняется в localStorage `midi_host_piano_roll_height_v1`. Максимальная высота теперь почти равна всей высоте `.main`: timeline может сжаться почти до нуля, сверху остаётся только toolbar секвенсора; старый лимит `70%` снят. Pitch canvas всегда содержит полный MIDI-диапазон 0–127; при открытии viewport центрируется на нотах clip, колесо мыши прокручивает регистр вверх/вниз, а увеличение высоты dock просто показывает больше строк одновременно без пустого поля. Live resize перерисовывается через `requestAnimationFrame`. Горизонтальный zoom внутри Piano Roll: `Ctrl/Cmd + wheel`; wheel down = zoom in/wider, wheel up = zoom out/narrower; браузерный zoom при наведении на Piano Roll блокируется, масштаб хранится отдельно в `midi_host_piano_roll_bar_w_v1` (24–8192 px/bar), zoom якорится около позиции курсора. Верхняя тактовая линейка Piano Roll поддерживает DAW-style gesture: обычный click или преимущественно horizontal drag двигает cue/playhead по выбранному snap; преимущественно vertical drag масштабирует горизонтальную ширину (`drag down` = zoom in/wider, `drag up` = zoom out/narrower), якорь масштаба остаётся под исходной X-позицией курсора. Вертикальный zoom не останавливает transport; экспоненциальный zoom теперь более плавный, чтобы один drag не упирался сразу в максимум.
- **Piano Roll v0.6 / нормальное MIDI-редактирование**: `Snap to Grid` теперь поддерживает `1/4`, `1/8`, `1/8T`, `1/16`, `1/16T`, `1/32`, `1/64` (default `1/16`, сохраняется в `midi_host_piano_roll_snap_v1`). Видимая сетка следует выбранному snap; triplet-сетка имеет отдельный визуальный тип, beat/bar остаются сильнее. Пустой Project Clip тоже открывается как редактируемая сетка и позволяет создавать ноты.
- Tool modes стандартизированы под Studio One/Fender Studio: `1` = Select/Arrow, `5` = Draw/Paint; при Draw удержание `Ctrl/Cmd` временно включает Select для box-selection. Режим виден в badge `EDIT · v0.6 · SELECT/DRAW`.
- Transpose-кнопки `−12/−1/+1/+12` удалены: выделенные ноты транспонируются стрелками Up/Down на ±1, Shift+Up/Down на ±12.
- Vertical zoom-кнопки удалены: `Ctrl/Cmd + wheel` над левой клавиатурой Piano Roll меняет высоту pitch rows; `Ctrl/Cmd + wheel` над сеткой остаётся horizontal zoom.
- Пустой Project Clip создаётся двойным кликом по свободному месту track lane: создаётся 1-bar `MIDI Part`, автоматически открывается Piano Roll в Draw tool.
- Горячие клавиши Piano Roll используют `KeyboardEvent.code` (`KeyA/KeyC/KeyV/KeyD/KeyZ/KeyY`, `Digit1/Digit5`), поэтому физические клавиши работают одинаково в EN/RU раскладках; Ctrl+A больше не должен уходить браузеру при открытом Piano Roll.
- Box/multi-select: drag по пустой сетке рисует marquee-рамку и выбирает пересекающиеся ноты; Ctrl/Cmd добавляет к текущему выделению. Ctrl/Cmd-click по ноте toggles её в selection. Drag выбранной ноты двигает всю выделенную группу по времени и pitch с текущим snap; границы Project Clip и MIDI 0–127 соблюдаются. Delete/Backspace удаляет всё выделение.
- Clipboard/edit shortcuts внутри открытого Piano Roll имеют приоритет над whole-clip shortcuts: `Ctrl/Cmd+C`, `Ctrl/Cmd+V`, `Ctrl/Cmd+D`, `Ctrl/Cmd+A`; Arrow Left/Right двигают выделение на один snap; Arrow Up/Down транспонируют ±1 semitone; Shift+Arrow Up/Down — ±12. Есть toolbar-кнопки Undo/Redo, Duplicate, −12/−1/+1/+12.
- Undo/Redo — локальная history конкретного открытого Project Clip, до 100 snapshots: `Ctrl/Cmd+Z`, `Ctrl/Cmd+Y` или `Ctrl/Cmd+Shift+Z`. History очищается при открытии/закрытии другого clip instance.
- Vertical zoom: высота pitch-row хранится в `midi_host_piano_roll_row_h_v1` (10–48 px); `Alt+wheel` и кнопки `V−/V+` меняют vertical scale с якорем около курсора. Horizontal zoom остаётся `Ctrl/Cmd+wheel` и ruler gesture.
- Audition: при drag выбранной ноты в остановленном transport изменение pitch коротко проигрывается live; create/paste/duplicate/transpose также audition'ятся. Если transport играет, commit редактирования перезапускает scheduler с текущего tick, поэтому новая версия слышна сразу после отпускания/команды.
- Piano Roll zoom mode также поддерживает horizontal pan без отпускания ЛКМ: после входа vertical drag'ом в zoom mode мышь вправо → содержимое Piano Roll визуально едет вправо, мышь влево → содержимое едет влево. Это совпадает с основным timeline и Studio One-style direct manipulation.
