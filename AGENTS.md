...
- Timeline markers отображаются как зелёные **флажки**. Обычный клик по флажку ставит cue на его такт; двойной клик или ПКМ открывает редактор полного названия с **Save / Copy name / Delete**. Максимальная ширина подписи автоматически равна доступному месту до следующего маркера, поэтому при широком zoom полное название видно сразу; троеточие остаётся только когда секция реально не помещается. При hover/focus подпись всё равно разворачивается целиком.
