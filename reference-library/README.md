...
Исходные `.zip`, `.tar.gz`, `.tgz`, `.tar` и отдельные `.mid/.midi` остаются raw-слоем и не переписываются. Поверх них bridge строит производную папку `<MIDI_REFERENCE_ROOT>/packs/`: небольшие ZIP-паки по схеме **style / role-type / source**, примерно до 1000 MIDI в одном архиве, плюс единый `library-index.json`.

Если `packs/library-index.json` отсутствует, первый запуск `start-midi-host.bat` автоматически один раз классифицирует raw corpus и собирает packs. После этого браузер строит список сразу из готового индекса, а для preview читает один маленький MIDI из соответствующего ZIP вместо повторного прохода по огромному TAR.GZ. Исходные архивы остаются нетронутыми и служат source of truth; производный слой можно пересобрать кнопкой **Rebuild packs** или командой `python scripts/rebuild_style_packs.py`.

В браузере можно искать и фильтровать референсы, слушать их через GM SoundFont, одновременно посылать на TD-3/XR20 и открывать выбранный MIDI в Hardware MIDI Host.
